...
export interface Employee {
  ...
  status: "Active" | "On Leave" | "Inactive" | "Draft";
  pan: string;
  aadhaar: string;
  uan: string;
  esi: string;
  ...
  ctc: number;
  avatar?: string;
  salaryStructure?: string | number | null;
  employeeType?: string;
  ...
  taxRegime?: string;
  taxSavingDeductions?: number | string;
}
...
export interface LeaveReq {
  id: string;
  employeeId: string;
  type: "Annual Leave" | "LOP";
  subType?: "Sick" | "Casual" | "Other";
  from: string;
  to: string;
  days: number;
  reason: string;
...
export interface SalaryComponent {
  ...
  calc: "Fixed" | "% of Basic" | "% of CTC";
  value: number;
}
export interface Loan {
  id: string;
  employeeId: string;
  type: "Personal" | "Salary Advance" | "Education";
  amount: number;
  emi: number;
  tenure: number;
  outstanding: number;
...
export interface Reimbursement {
  id: string;
  employeeId: string;
  category: "Travel" | "Meals" | "Internet" | "Medical";
  amount: number;
  date: string;
...
function seed() {
  const entities: Entity[] = [
    ...
    },
    {
      ...
      country: "India",
      currency: "INR",
      gstin: "29AABCA1234B1Z2",
      status: "Active",
    },
  ];
  const branches: Branch[] = CITIES.map(([city, st], i) => ({
    id: id("br", i + 1),
    entity: entities[i % 3].id,
    name: `${city} Branch`,
    code: `BR-${city.slice(0, 3).toUpperCase()}`,
    city,
    state: st,
    head: `${rand(FIRST)} ${rand(LAST)}`,
  }));
  const sites: Site[] = branches.flatMap((b, i) => [
    {
      ...
      branch: b.id,
      name: `${b.city} HQ`,
      address: `${randInt(1, 500)} Tech Park, ${b.city}`,
      latitude: 12.9 + i * 0.3,
      longitude: 77.5 + i * 0.4,
      radius: 150,
      qrEnabled: true,
      faceEnabled: true,
    },
    {
      ...
      branch: b.id,
      name: `${b.city} Annexe`,
      address: `${randInt(1, 500)} Business Bay, ${b.city}`,
      latitude: 12.9 + i * 0.3 + 0.01,
      longitude: 77.5 + i * 0.4 + 0.01,
      radius: 100,
      qrEnabled: true,
      faceEnabled: false,
    },
  ]);
  const departments: Department[] = DEPTS.map((d, i) => ({
    id: id("dept", i + 1),
    entity: entities[i % 3].id,
    name: d,
  ...
  const designations: Designation[] = departments.flatMap((d, i) =>
    DESIGS.slice(0, 4).map((t, j) => ({
      id: id("desg", i * 4 + j + 1),
      department: d.id,
      title: t,
      grade: `G${j + 3}`,
  ...
  const employees: Employee[] = Array.from({ length: 48 }).map((_, i) => {
    ...
    return {
      ...
      firstName: f,
      lastName: l,
      email: `${f.toLowerCase()}.${l.toLowerCase()}@acme.com`,
      phone: `+91 9${randInt(100000000, 999999999)}`,
      entity: String(dept.entity),
      branch: br.id,
      site: site.id,
      department: dept.id,
      designation: desg.id,
      ...
      status: rand(["Active", "Active", "Active", "On Leave"] as const),
      pan: `ABCDE${randInt(1000, 9999)}F`,
      aadhaar: `${randInt(1000, 9999)} ${randInt(1000, 9999)} ${randInt(1000, 9999)}`,
      uan: `${randInt(100000000000, 999999999999)}`,
      esi: `${randInt(1000000000, 9999999999)}`,
      bankName: rand(["HDFC Bank", "ICICI Bank", "Axis Bank", "SBI", "Kotak"]),
      bankAccount: `${randInt(10000000000, 99999999999)}`,
  ...
  const leaves: LeaveReq[] = Array.from({ length: 18 }).map((_, i) => {
    ...
    const from = new Date();
    from.setDate(from.getDate() + randInt(-10, 20));
    const to = new Date(from);
    to.setDate(from.getDate() + randInt(0, 4));
    return {
      id: id("lv", i + 1),
      employeeId: emp.id,
      type: rand(["Annual Leave", "LOP"] as const),
      subType: rand(["Casual", "Sick", "Other"] as const),
  ...
  const payrollRuns: PayrollRun[] = ["2026-04", "2026-05", "2026-06"].map((p, i) => ({
    ...
    gross: 28500000 + i * 100000,
    deductions: 4200000,
    net: 24300000 + i * 100000,
    status: i === 2 ? "Draft" : "Disbursed",
    runOn: `${p}-28`,
  }));
  ...
  const loans: Loan[] = Array.from({ length: 10 }).map((_, i) => {
    ...
    return {
      id: id("ln", i + 1),
      employeeId: emp.id,
      type: rand(["Personal", "Salary Advance", "Education"] as const),
      amount,
      emi: Math.round(amount / randInt(12, 36)),
      tenure: randInt(12, 36),
      outstanding: Math.round(amount * Math.random()),
      status: rand(["Active", "Active", "Closed", "Pending"] as const),
    };
  });
  ...
  const reimbursements: Reimbursement[] = Array.from({ length: 14 }).map((_, i) => {
    ...
    return {
      id: id("rb", i + 1),
      employeeId: emp.id,
      category: rand(["Travel", "Meals", "Internet", "Medical"] as const),
      amount: randInt(500, 25000),
      date: `2026-06-${String(randInt(1, 17)).padStart(2, "0")}`,
      status: rand(["Pending", "Approved", "Paid", "Rejected"] as const),
      receipt: `RCPT-${1000 + i}.pdf`,
    };
  });
  ...
  const regularizations: Regularization[] = Array.from({ length: 8 }).map((_, i) => {
    ...
    return {
      id: id("rg", i + 1),
      employeeId: emp.id,
      date: `2026-06-${String(randInt(1, 15)).padStart(2, "0")}`,
      reason: rand([
        "Forgot to check-in",
        "System issue",
      ...
      requestedIn: `09:${String(randInt(0, 59)).padStart(2, "0")}`,
      requestedOut: `18:${String(randInt(0, 59)).padStart(2, "0")}`,
...
export function empName(empId?: string) {
