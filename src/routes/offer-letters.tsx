...
function OfferLettersDashboard() {
  ...
  const stats = [
    {
      label: "Draft",
      value: data?.metrics?.["draft"] || data?.metrics?.["Draft"] || 0,
      icon: FileText,
      color: "text-muted-foreground",
    },
    {
      label: "Pending Approval",
      value: data?.metrics?.["pendingApproval"] || data?.metrics?.["Pending Approval"] || 0,
      icon: Clock,
      color: "text-amber-500",
    },
    {
      label: "Awaiting Acceptance",
      value: data?.metrics?.["awaitingAcceptance"] || data?.metrics?.["Awaiting Acceptance"] || 0,
      icon: Send,
      color: "text-blue-500",
    },
    {
      label: "Accepted",
      value: data?.metrics?.["accepted"] || data?.metrics?.["Accepted"] || 0,
      icon: CheckCircle2,
      color: "text-emerald-500",
    },
    {
      label: "Rejected",
      value: data?.metrics?.["rejected"] || data?.metrics?.["Rejected"] || 0,
      icon: XCircle,
      color: "text-red-500",
    },
    {
      label: "Expired",
      value: data?.metrics?.["expired"] || data?.metrics?.["Expired"] || 0,
      icon: Timer,
      color: "text-slate-500",
    },
    {
      label: "Joined",
      value: data?.metrics?.["joined"] || data?.metrics?.["Joined"] || 0,
      icon: UserCheck2,
      color: "text-emerald-600",
    },
    {
      label: "Declined",
      value: data?.metrics?.["declined"] || data?.metrics?.["Declined"] || 0,
      icon: Ban,
      color: "text-rose-500",
    },
  ];
  ...
  return (
    <>
      <div className="flex justify-between items-end mb-6">
        <PageHeader
          title="Offer Letters"
          description="Pipeline view across all stages — from draft to joined."
        />
        <Link
          to="/all-offers"
          className="bg-primary text-primary-foreground hover:bg-primary/90 h-9 px-4 inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors"
        >
            ...
            {stats.map((stat, i) => (
              ...
              >
                <div className="space-y-1">
                  ...
                  <p
                    className={`text-2xl font-bold ${stat.value > 0 ? stat.color : "text-foreground"}`}
                  >
                    {stat.value}
                  </p>
                </div>
              ...
              <div className="divide-y flex-1">
                {data?.recent?.map((offer: any) => (
                  <div
                    ...
                    onClick={() =>
                      navigate({ to: "/employees/$id", params: { id: String(offer.employee) } })
                    }
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-muted-foreground text-xs">
                          {offer.designationName || offer.designation_name}
                        </span>
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">
                        {offer.offerNumber || offer.offer_number} · Joining{" "}
                        {offer.joiningDate || offer.joining_date || "TBD"}
                      </div>
                    </div>
                    <div>
                      <span
                        className={`px-2.5 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider ${getStatusColor(offer.status)}`}
                      >
                        {offer.status}
                      </span>
                    </div>
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground mt-0.5">
                        {offer.departmentName || offer.department_name || "N/A"}
                      </div>
                    </div>
