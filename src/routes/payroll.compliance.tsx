...
function CompliancePage() {
  ...
  const nextDue =
    pending.length > 0
      ? new Date(Math.min(...pending.map((r: any) => new Date(r.due).getTime())))
      : null;
  const daysRemaining = nextDue
    ? Math.ceil((nextDue.getTime() - new Date().getTime()) / (1000 * 3600 * 24))
    : 0;
  ...
  const lastFiled =
    filed.length > 0
      ? [...filed].sort(
          (a: any, b: any) =>
            new Date(b.filedOn || b.filed_on).getTime() -
            new Date(a.filedOn || a.filed_on).getTime(),
        )[0]
      : null;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Track PF, ESI, Professional Tax and TDS liabilities, generate returns and download
            challans.
          </p>
        </div>
        <div className="flex gap-3">
          <Dialog>
            <DialogTrigger asChild>
              <Button
                variant="outline"
                className="text-slate-700 bg-white shadow-sm border-slate-200"
              >
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <Badge className="bg-[#dcfce7] text-[#166534] hover:bg-[#dcfce7] shadow-none border-0 text-xs">
                Healthy
              </Badge>
            </div>
            <p className="text-sm text-[#166534]">
              All filings up to October are submitted. Next critical due date is 15 Dec (PF & ESI).
            </p>
          </div>
        </div>
        <Button
          variant="ghost"
          className="text-[#166534] hover:text-[#14532d] hover:bg-[#dcfce7] text-sm font-medium"
        >
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <Card className="p-6 shadow-sm border-[#fde68a] bg-[#fefce8]">
          ...
          <div className="text-3xl font-bold text-amber-900">
            {nextDue
              ? nextDue.toLocaleDateString("en-GB", { day: "numeric", month: "short" })
              : "None"}
          </div>
          <div className="text-xs text-amber-700 mt-2">
            {daysRemaining > 0 ? `${daysRemaining} days remaining` : "On track"}
          </div>
        </Card>
        ...
        <Card className="p-6 shadow-sm border-slate-200">
          ...
          <div className="text-3xl font-bold text-slate-900">
            {lastFiled ? lastFiled.period : "—"}
          </div>
          <div className="text-xs text-[#16a34a] mt-2">
            {lastFiled
              ? `On time · ${new Date(lastFiled.filed_on).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}`
              : "No history"}
          </div>
        </Card>
        ...
        <Card className="p-6 shadow-sm border-slate-200">
          ...
          <div className="text-lg font-bold text-slate-900 mt-2 tracking-wide">
            {REGISTRATIONS[activeTab]}
          </div>
          <div className="text-xs text-blue-600 font-medium mt-3 cursor-pointer hover:underline">
            Edit details
          </div>
        </Card>
      </div>
      ...
      <Card className="shadow-sm border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white">
          ...
          <Button
            ...
            onClick={() => toast.success("export successfully!")}
          >
            Export CSV
          </Button>
        </div>
                    ...
                    <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <Badge
                          className={
                            r.status === "Filed"
                              ? "bg-[#dcfce7] text-[#166534] hover:bg-[#dcfce7] shadow-none border-0"
                              : r.status === "Pending"
                                ? "bg-[#fef3c7] text-[#92400e] hover:bg-[#fef3c7] shadow-none border-0"
                      ...
                      <td className="px-6 py-4 text-slate-500">
                        {r.filedOn || r.filed_on
                          ? new Date(r.filedOn || r.filed_on).toLocaleDateString("en-GB", {
                              day: "numeric",
                              month: "short",
                              year: "numeric",
                            })
                      ...
                      <td className="px-6 py-4 text-right">
                        <Button
                          ...
                          onClick={() => toast.success(`Downloaded ${r.period} challan`)}
                        >
