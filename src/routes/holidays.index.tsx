    ...
    holidaysApi
      ...
      .then((res: any) => {
        ...
        setStats({
          ...
          regional_festival: allHolidays.filter(
            (h: any) => h.holiday_type === "Regional" || h.holiday_type === "Festival",
          ).length,
        });
      })
      .catch(console.error);
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div
          onClick={() => setFilterType(filterType === "TOTAL" ? null : "TOTAL")}
          className="cursor-pointer transition-transform hover:scale-[1.02]"
        >
          <StatCard
            label="TOTAL HOLIDAYS"
            value={(stats?.total_holidays || 0).toString()}
            icon={Calendar}
          />
        </div>
        <div
        ...
        <div
          onClick={() => setFilterType(filterType === "OPTIONAL" ? null : "OPTIONAL")}
          className="cursor-pointer transition-transform hover:scale-[1.02]"
        >
          <StatCard
            label="OPTIONAL"
            value={(stats?.optional || 0).toString()}
            icon={CalendarOff}
            tone="warning"
          />
        </div>
        <div
          onClick={() => setFilterType(filterType === "RESTRICTED" ? null : "RESTRICTED")}
          className="cursor-pointer transition-transform hover:scale-[1.02]"
        >
          <StatCard
            label="RESTRICTED"
            value={(stats?.restricted || 0).toString()}
            icon={Users}
            tone="warning"
          />
        </div>
        <div
          onClick={() => setFilterType(filterType === "REGIONAL" ? null : "REGIONAL")}
          className="cursor-pointer transition-transform hover:scale-[1.02]"
        >
          <StatCard
            label="REGIONAL / FESTIVAL"
            value={(stats?.regional_festival || 0).toString()}
            icon={Map}
          />
        </div>
      </div>
          ...
          <div className="space-y-4 mt-4">
            {holidays.filter((h) => {
              ...
              if (filterType === "REGIONAL")
                return h.holiday_type === "Regional" || h.holiday_type === "Festival";
              return true;
            }).length === 0 ? (
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant={holiday.holiday_type === "Company" ? "default" : "secondary"}
                          className={
                            holiday.holiday_type === "Company"
                              ? "bg-[#c3f0ca] text-[#1c7b36] hover:bg-[#c3f0ca]/80"
