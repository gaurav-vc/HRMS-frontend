...
function QrPage() {
  ...
  return (
    <>
      <PageHeader
        title="Dynamic QR Kiosk"
        description="Auto-rotating site QR — mount this device at the secure entrance."
      />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="p-8 flex flex-col items-center justify-center text-center shadow-[var(--shadow-elegant)] min-h-[500px]">
          <div className="text-xs uppercase tracking-widest text-muted-foreground mb-4">
            Bengaluru HQ • Secure Gateway
          </div>

          <div className="p-6 rounded-2xl bg-white border-4 border-primary/10 shadow-inner relative">
            <div className="absolute top-2 right-2 flex space-x-1">
              <div
                className={`w-2 h-2 rounded-full ${counter > 10 ? "bg-success" : "bg-destructive animate-pulse"}`}
              />
            </div>
            {token ? (
              <QRCodeSVG
          ...
          <div className="mt-4 flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>
              Refreshes in <strong className="text-foreground">{counter}s</strong>
            </span>
          </div>
        </Card>
          ...
          <Card className="p-6">
            ...
            <ul className="space-y-2 text-sm text-muted-foreground mt-4">
              <li className="flex items-start gap-2">
                ...
                <p>
                  <strong>Cryptographic TTL:</strong> Token rotates every 120 seconds to prevent
                  physical photograph replay attacks.
                </p>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5" />
                <p>
                  <strong>Single-Use Constraint:</strong> Burned immediately per employee. Cannot be
                  replayed by the same individual.
                </p>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5" />
                <p>
                  <strong>Hardware Geofencing:</strong> Enforces Haversine proximity checks during
                  punch sequence.
                </p>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5" />
                <p>
                  <strong>Biometric Fusion:</strong> Combined with Liveness Check and DeepFace ONNX
                  matching on the employee device.
                </p>
              </li>
            </ul>
          </Card>
