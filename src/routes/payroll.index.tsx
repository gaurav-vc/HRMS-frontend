  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        ...
        <StatCard label="Est. Net Pay" value={canViewConfidential ? fmtINR(last.net) : "***"} delta="+0.8%" icon={Wallet} tone="success" />
        <StatCard label="Pending Reimb." value={String(reimbursements.filter((r: any) => r.status === "Pending").length)} icon={TrendingUp} tone="warning" />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card className="p-5"><h3 className="font-semibold mb-3">Recent Payroll Runs</h3>
          <ul className="divide-y">{payrollRuns.slice().reverse().map((r: any) => (
            <li key={r.id} className="py-3 flex items-center justify-between">
              <div><div className="font-medium">{r.period}</div><div className="text-xs text-muted-foreground">{r.employees} employees • {canViewConfidential ? fmtINR(r.net) : "***"} net</div></div>
