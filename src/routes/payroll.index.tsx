...
import { payrollApi, loansApi, reimbursementsApi, employeesApi } from "@/api";
...
function PayrollOverview() {
  ...
  const data = payrollRuns.map((r: any) => ({
    name: r.period.slice(5),
    gross: r.gross / 100000,
    net: r.net / 100000,
  }));
  ...
  return (
    <>
      <PageHeader
        ...
        description="Multi-entity payroll command center"
        actions={
          <Link to="/payroll/run">
            <Button>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        ...
        <StatCard
          label="Est. Net Pay"
          value={canViewConfidential ? fmtINR(last.net) : "***"}
          delta="+0.8%"
          icon={Wallet}
          tone="success"
        />
        <StatCard
          label="Pending Reimb."
          value={String(reimbursements.filter((r: any) => r.status === "Pending").length)}
          icon={TrendingUp}
          tone="warning"
        />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3 mb-6">
        <Link to="/payroll/structure">
          <Card className="p-5 hover:border-primary/40 transition">
            ...
            <div className="font-semibold">Salary Structure</div>
            <div className="text-xs text-muted-foreground mt-1">
              Define earnings, deductions and components
            </div>
          </Card>
        </Link>
        <Link to="/payroll/slips" search={{ period: undefined }}>
          <Card className="p-5 hover:border-primary/40 transition">
            <ReceiptText className="h-6 w-6 text-primary mb-2" />
            <div className="font-semibold">Salary Slips</div>
            <div className="text-xs text-muted-foreground mt-1">
              View, download and email payslips
            </div>
          </Card>
        </Link>
        <Link to="/payroll/compliance">
          <Card className="p-5 hover:border-primary/40 transition">
            ...
            <div className="text-xs text-muted-foreground mt-1">PF, ESI, PT and TDS filings</div>
          </Card>
        </Link>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          <h3 className="font-semibold mb-3">Gross vs Net (₹ Lakhs)</h3>
        ...
        <Card className="p-5">
          <h3 className="font-semibold mb-3">Recent Payroll Runs</h3>
                ...
                <li key={r.id} className="py-3 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {r.employees} employees • {canViewConfidential ? fmtINR(r.net) : "***"} net
                    </div>
                  </div>
                  <Badge
                    className={
      ...
      <Card className="p-5 mt-6">
        <h3 className="font-semibold mb-3">Active Loans Snapshot</h3>
        <ul className="divide-y">
          {loans.slice(0, 6).map((l) => (
            <li key={l.id} className="py-2 flex justify-between text-sm">
              <span>
                {l.type} • {fmtINR(l.amount)}
              </span>
              <span className="text-muted-foreground">
                EMI {fmtINR(l.emi)} • {l.tenure}m
              </span>
            </li>
          ))}
        </ul>
