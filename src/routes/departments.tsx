  ...
  return (
    <>
      ...
      <DataTable
        ...
        searchKeys={["name", "head", "code"]}
        filters={[
          {
            label: "Entity",
            key: "entityId",
            options: entities.map((e) => ({ value: String(e.id), label: e.code })),
        ...
        columns={[
          ...
          },
          {
            key: "entity",
            header: "Entity",
            accessor: (r) => entities.find((e) => String(e.id) === String(r.entity))?.code ?? "—",
            render: (r) => entities.find((e) => String(e.id) === String(r.entity))?.code ?? "—",
          },
          {
            key: "head",
            header: "HOD",
            accessor: (r) => {
              const headEmp: any = employees.find(
                (e: any) => String(e.id) === String(r.head) || String(e.code) === String(r.head),
              );
              return headEmp
                ? `${headEmp.firstName || headEmp.first_name} ${headEmp.lastName || headEmp.last_name}`
                : r.head || "—";
            },
            render: (r) => {
              const headEmp: any = employees.find(
          ...
          },
          {
            ...
            header: "Headcount",
            accessor: (r) =>
              employees.filter((e: any) => String(e.department) === String(r.id)).length,
            render: (r) =>
              employees.filter((e: any) => String(e.department) === String(r.id)).length,
          },
          {
...
function DepartmentDialog({
  ...
  return (
    <Dialog
      ...
      onOpenChange={(b) => {
        onOpenChange(b);
        if (b && department) setForm(department);
      }}
    >
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {mode === "view" ? "View" : department ? "Edit" : "Create"} Department
          </DialogTitle>
        </DialogHeader>
        <div className="grid gap-3">
          <Field label="Entity">
            <Select
              ...
              onValueChange={(v) => setForm({ ...form, entity: v } as any)}
              disabled={mode === "view"}
            >
              <SelectTrigger>
                ...
                <Command>
                  ...
                  <CommandGroup>
                    <div className="max-h-[200px] overflow-y-auto">
                      ...
                      >
                        <Check
                          className={`mr-2 h-4 w-4 ${!form.head ? "opacity-100" : "opacity-0"}`}
                        />
                        None
                      </CommandItem>
                      {(employees || []).map((e: any) => (
                        ...
                        >
                          <Check
                            className={`mr-2 h-4 w-4 ${String(form.head) === String(e.id) ? "opacity-100" : "opacity-0"}`}
                          />
