      ...
      <DataTable
        ...
        columns={[
          ...
          { key: "grade", header: "Grade", accessor: (r) => r.grade, sortable: true },
          {
            ...
            header: "Department",
            accessor: (r) =>
              departments.find((d) => String(d.id) === String(r.department))?.name ?? "—",
            render: (r) =>
              departments.find((d) => String(d.id) === String(r.department))?.name ?? "—",
          },
          {
...
function DesignationDialog({
  ...
  return (
    <Dialog
      ...
      onOpenChange={(b) => {
        onOpenChange(b);
        if (b && designation) setForm(designation);
      }}
    >
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {mode === "view" ? "View" : designation ? "Edit" : "Create"} Designation
          </DialogTitle>
        </DialogHeader>
