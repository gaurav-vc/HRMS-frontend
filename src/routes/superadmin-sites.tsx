...
export const Route = createFileRoute("/superadmin-sites")({
  ...
  loader: async () => {
    try {
      const [sites, orgs] = await Promise.all([sitesApi.getAll(), organizationsApi.getAll()]);
      const sitesList = (sites as any)?.results || sites || [];
      const sortedSites = sitesList.sort(
        (a: any, b: any) =>
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[15px] text-slate-500 mt-1">
            Geofenced work locations and branch configuration
          </p>
        </div>
        <Button
          className="bg-[#1a4cd2] hover:bg-[#1641b4] text-white shadow-sm"
          onClick={() => {
            setEditing(null);
            setMode("create");
            setOpen(true);
        ...
        <DataTable
          ...
          columns={[
            ...
            },
            {
              ...
              render: (r: any) => (
                <span className="text-muted-foreground">{r.address || r.city || "—"}</span>
              ),
            },
            {
              key: "productType",
            ...
            },
            {
              ...
              render: (r: any) => (
                <Badge
                  ...
                  className={r.status === "Active" ? "bg-emerald-100 text-emerald-800" : ""}
                >
                  {r.status || "Active"}
                </Badge>
              ),
...
function SiteDialog({ open, onOpenChange, site, onSave, organizations, mode, lockedOrg }: any) {
  ...
  const handleAddModule = () => {
    ...
    if (!exists) {
      setForm({
        ...form,
        modules: [...mods, { name: mod, view: true, create: false, update: false, delete: false }],
      });
  ...
  const customItems = (form.modules || [])
    .filter((m: any) => {
  ...
  const displayedModules = allModulesList
    .map((g) => ({
      ...g,
      ...
      <div className="p-6 pb-4 border-b flex items-center justify-between gap-4 bg-slate-50/50 rounded-t-xl">
        ...
        {mode === "create" && (
          <Button
            ...
            size="sm"
            onClick={fillDummy}
            className="ml-auto bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border-indigo-200"
          >
            Fill Dummy Data
          </Button>
        )}
              ...
              {lockedOrg ? (
                <div className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-muted px-3 py-2 text-sm ring-offset-background">
                  <span className="text-muted-foreground">
                    {organizations.find((o: any) => o.id === Number(lockedOrg))?.name || lockedOrg}
                  </span>
                </div>
              ) : (
                <Select
                  disabled={mode === "view"}
                  value={form.organization ? form.organization.toString() : ""}
                  onValueChange={(v) => setForm({ ...form, organization: Number(v) })}
                >
                  <SelectTrigger>
        ...
        <section className="bg-white p-6 rounded-lg border shadow-sm space-y-6">
          ...
          <div className="flex gap-4 items-center">
            ...
            <Button
              ...
              disabled={mode === "view" || !moduleSearch.trim()}
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-4 gap-x-6">
                  {group.items.map((mod) => {
                    ...
                    return (
                      <div
                        key={mod}
                        className="flex items-center space-x-2 border p-3 rounded-md bg-white"
                      >
                        <Checkbox
                          checked={isSelected}
