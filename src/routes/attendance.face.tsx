...
function ScanPunchPage() {
  ...
  const sitesList = Array.isArray(loaderData.sites)
    ? loaderData.sites
    : (loaderData.sites as any).results || [];
  ...
  const [step, setStep] = useState<ScanStep>(
    sitesList.length === 0 || loaderError
      ? "ERROR"
      : sitesList.length === 1
        ? sitesList[0].qrEnabled === false
          ? "FACE_LIVENESS"
          : "QR"
        : "SELECT_SITE",
  );
  ...
  const [errorMsg, setErrorMsg] = useState(
    loaderError
      ...
      : sitesList.length === 0
        ? "No site assigned to your profile. Backend returned 0 sites."
        : "",
  );
    ...
    const fetchChallenge = async () => {
      try {
        ...
        if (chalId) {
          ...
          const timer1 = setTimeout(() => setLivenessPrompt(`Step 2: ${data.steps[1]}`), 2500);
          const timer2 = setTimeout(() => setLivenessPrompt("Perfect. Capturing..."), 5000);
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        if (err.code === 1)
          msg =
            "Browser GPS Permission Denied. Please allow location access in your browser settings.";
  ...
  useEffect(() => {
    ...
    const submitPunch = async () => {
      try {
        ...
        const data = await attendanceApi.punchWithFace(
          payload,
          new File([faceImageBlob], "face.jpg", { type: "image/jpeg" }),
        );
          ...
          <div className="relative aspect-[3/4] sm:aspect-square bg-slate-950 rounded-xl overflow-hidden border-2 border-slate-800 flex flex-col items-center justify-center">
            {step === "SELECT_SITE" && (
              <div className="w-full h-full p-8 flex flex-col items-center justify-center bg-white text-slate-900 absolute inset-0 z-20">
                ...
                <p className="text-sm text-muted-foreground text-center mb-6">
                  Please confirm the site you are punching from.
                </p>
            ...
            {step === "QR" && (
              <div className="w-full h-full p-4 relative">
                <div className="absolute top-4 left-0 w-full text-center z-10 text-white font-medium drop-shadow-md">
                  Scan Kiosk QR Code
                </div>
                <Scanner
                  onScan={handleQrScan}
              ...
              >
                ...
                {site && (
                  <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
                    ...
                    <div className="text-center mt-2 text-xs font-semibold text-slate-700">
                      Site geofence
                    </div>
                  </div>
                )}
                {location && site && (
                  <div
                    ...
                    style={{
                      left: `${50 + (location.lng - site.longitude) * 30000}%`,
                      top: `${50 - (location.lat - site.latitude) * 30000}%`,
                    }}
                  >
                    <div className="w-4 h-4 rounded-full bg-success border-2 border-white shadow-lg -translate-x-1/2 -translate-y-1/2" />
                    <div className="text-xs font-bold mt-1 px-2 py-0.5 rounded bg-white border shadow-sm absolute left-1/2 -translate-x-1/2">
                      You
                    </div>
                  </div>
                )}
            ...
            {step === "SUBMITTING" && (
              <div className="text-center text-white p-6">
                ...
                <p className="text-sm text-slate-400 mt-2">
                  Running DeepFace embeddings & cryptographic checks...
                </p>
              </div>
            )}
            ...
            {step === "DONE" && (
              <div className="text-center text-white p-6 bg-success/10 w-full h-full flex flex-col justify-center">
                ...
                <div className="bg-slate-900/50 p-4 rounded-xl inline-block mt-4 text-left border border-slate-800">
                  <p className="text-sm">
                    <span className="text-slate-400 w-24 inline-block">QR Token:</span>{" "}
                    <span className="text-success">Validated</span>
                  </p>
                  <p className="text-sm mt-1">
                    <span className="text-slate-400 w-24 inline-block">Geofence:</span>{" "}
                    <span className="text-success">Match (±15m)</span>
                  </p>
                  <p className="text-sm mt-1">
          ...
          <Card className="p-6 border-l-4 border-l-primary">
            ...
            <ul className="space-y-3 text-sm text-muted-foreground mt-4">
              <li>
                ...
                <p className="mt-1">
                  Raw facial images are processed in-memory and destroyed immediately after vector
                  extraction. Images are never written to disk.
                </p>
              </li>
              <li>
                <strong className="text-foreground">Liveness Anti-Spoofing:</strong>
                <p className="mt-1">
                  Hardware requires blink and yaw detection to prevent static photo replay attacks.
                </p>
              </li>
              <li>
                <strong className="text-foreground">Cryptographic Isolation:</strong>
                <p className="mt-1">
                  Punches require a physically proximate rotating QR token coupled with encrypted
