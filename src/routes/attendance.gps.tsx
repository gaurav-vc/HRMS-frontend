...
function GpsPage() {
  ...
  const distance = Math.round(
    Math.sqrt((pos.lat - site.latitude) ** 2 + (pos.lng - site.longitude) ** 2) * 111000,
  );
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="p-6 lg:col-span-2 shadow-[var(--shadow-elegant)]">
          <div
            ...
            style={{
              background: "linear-gradient(145deg, oklch(0.95 0.02 200), oklch(0.92 0.04 160))",
            }}
            onClick={(e) => {
          ...
          >
            <svg className="absolute inset-0 w-full h-full pointer-events-none">
              <defs>
                <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
                  <path d="M40 0H0V40" fill="none" stroke="oklch(0.85 0.02 200)" strokeWidth="1" />
                </pattern>
              </defs>
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
              ...
              <div className="text-center mt-2 text-xs text-muted-foreground">
                Site geofence ({site.radius}m)
              </div>
            </div>
            <div
              className="absolute"
              style={{
                left: `${50 + (pos.lng - site.longitude) * 30000}%`,
                top: `${50 - (pos.lat - site.latitude) * 30000}%`,
              }}
            >
              <div className="w-4 h-4 rounded-full bg-success border-2 border-white shadow-lg" />
              <div className="text-xs font-medium mt-1 px-2 py-0.5 rounded bg-card border shadow-sm">
                You
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-2 mt-4">
                ...
                if ("geolocation" in navigator) {
                  ...
                  navigator.geolocation.getCurrentPosition(
                    ...
                    (error) => {
                      ...
                      if (error.code === 1)
                        msg =
                          "Browser GPS Permission Denied. Please allow location access in your browser settings (the lock icon in the URL bar).";
            ...
            <Button disabled={!inside || loading} onClick={() => handlePunch("IN")}>
        ...
        <Card className="p-6">
          ...
          <div className="mt-4 space-y-2 text-xs">
            ...
            <div className="flex justify-between">
              ...
              >
                {site.qrEnabled ? "Enabled" : "Disabled"}
              </Badge>
            </div>
            <div className="flex justify-between">
              ...
              >
                {site.faceEnabled ? "Enabled" : "Disabled"}
              </Badge>
            </div>
            <div className="flex justify-between">
              ...
              >
                {inside ? "Inside geofence" : "Outside"}
              </Badge>
            </div>
          </div>
        </Card>
