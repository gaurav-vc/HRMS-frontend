...
function RosterPage() {
  ...
  const [roster, setRoster] = useState<any[]>(
    Array.isArray(rosterData) ? rosterData : (rosterData as any)?.results || [],
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card p-4 rounded-xl border flex items-center gap-4">
              ...
              {actualShifts.map((s: any) => (
                <SelectItem key={s.id} value={s.id.toString()}>
                  <div className="flex items-center gap-2">
                    ...
                    />
                    {s.name} ({(s.startTime || "00:00").substring(0, 5)}–
        ...
        <span className="text-sm text-muted-foreground ml-auto">
          Clicking a cell assigns or replaces; click an existing block's × to clear.
        </span>
      </div>
      ...
      <div className="border rounded-xl bg-card overflow-x-auto custom-scrollbar">
        <table className="w-full text-sm text-left">
          <thead className="bg-muted/50 border-b">
            <tr>
              <th className="px-4 py-3 font-medium text-muted-foreground border-r w-[250px]">
                Employee
              </th>
              {days.map((d) => {
          ...
          <tbody className="divide-y">
            {roster.map((row) => (
              <tr
                key={row.employee?.id || Math.random()}
